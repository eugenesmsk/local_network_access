...
- Для установки Wireguard я предлагаю использовать скрипт, взятый [отсюда](https://github.com/DigneZzZ/dwg) и использовать установку без графического интерфейса и wg-easy. Обращаю внимание, что при использовании wg-easy при перезагрузке докер-контейнера пересоздаётся конфигурационный файл wg0.conf, что помешает нам отредактировать конфиг таким образом, чтобы был доступ в локальную сеть.
  ...
  Чтобы пользоваться VPN'ом достаточно скачать Wireguard клиент. На смартфоне можно просто отсканировать QR-код и все настройки на клиенте автоматически подтянутся. На Windows клиенте нужно сохранить конфигурационный файл.
  Нас же интересует на данном этапе настройка роутера, поэтому мы скопируем эти настройки
  
  ![image](https://github.com/eugenesmsk/local_network_access/assets/41268173/e32fae9e-bb67-4e63-9d11-2a535dbb63ce)
  
  Далее с поиощью notepad++ или любого другого редактора необходимо сохранить их в файл router.conf.
  
  В папке dwg/tools есть различные скрипты. [Тут](https://github.com/DigneZzZ/dwg) можете ознакомиться что они значат и определить нажны ли они вам. Для запуска скрипта вводим `bash ИМЯ_СКРИПТА.sh`
  
   ---
    Небольшое отступеление по поводу скорости интернета через Wireguard. Попробуйте подключиться к Wireguard с телефона или компьютера и измерить скорость. Если со скоростью всё ОК, можете переходить к настройке роутера Keenetic. Если же скорость существенно режется, то рекомендую в приложении Wireguard добавить в конфигурацию клиента MTU = 1280 в разделе Interface, а также в файле wg0.conf добавить в раздел Interface такую же строку:
    ```
...
## Настройка роутера Keenetic:
  ...
  После этого в боковой панели нажимаем Другие подключение во вкладке Интернет.
  ...
  Далее рекомендую создать . Для этого переходим в боковой панели выбираем Приоритеты подключений во вкладке Интернет и создаём новую политику, как показано на скриншоте ниже:
  
  ![image](https://github.com/eugenesmsk/local_network_access/assets/41268173/78c340b2-792b-4864-8a74-56d9a183401d)
  
  Теперь в разделе Применение политик можно удобно распределять какие из зарегестрированных устройств домашней сети будут выходить в интернет через VPN, а какие нет.Также профиль доступа для каждого из устройств можно менять в приложении Keenetic.

## Настрока доступа к локальной сети
Теперь мы хотим получить доступ к устройсвам локальной сети по их локальным адресам. Это может быть очень удобно, если у вас есть какое-то облачное хранилище, сервер, умный дом и т.д.
Для этого в боковом меню перейдём в Межсетевой экран во вкладке Сетевые правила. Далее выбираем вкладку router и нажимаем Добавить правило. Добавьте настройки в соответствии со скиншотом ниже, заменив IP-адрес источника на ваш. Узнать ваш адрес подсети можно в файле router.conf в разделе Interface, который мы ранее сохраняли. В нашем случае `Address = 10.10.10.4`, значит адрес подсети `10.10.10.0`. Адрес назначения - адрес нашей локальной подсети: `192.168.1.0`. Не забудьте включить правило, установив соответствующую галочку.
...
Нажимаем Сохранить и всё - настройка завершена. Можно пробовать подключаться. Для этого активируем VPN-подключение в клиенте Wireguard и можем пипнгануть адрес нашего роутера:
